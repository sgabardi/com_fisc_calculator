...
class CalculatorModel extends ItemModel
{
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('calculator');
		}
...
	public function getCalculatorFields() {		$id = (!empty($id)) ? $id : (int) $this->getState('calculator.id');				$db = Factory::getContainer()->get('DatabaseDriver');				$query = $db->getQuery(true);				$query->select('*');
		$query->from('#__fisc_calculator_item');		$query->where('calculator_id = '.$db->quote($id));		$query->order('ordering');
