 ...
 * @since  1.0.0
 */
class CalculatoritemformModel extends FormModel
{
	private $item = null;
	...
	public function getItem($id = null)
	{
		if ($this->item === null)
		{
			...
			if ($table !== false && $table->load($id) && !empty($table->id))
			{
				...
				if (!$canEdit && $user->authorise('core.edit.own', 'com_fisc_calculator'))
				{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_fisc_calculator.calculatoritem', 'calculatoritemform', array(
						'control'   => 'jform',
						'load_data' => $loadData
				)
		);
	...
	{
		$data = Factory::getApplication()->getUserState('com_fisc_calculator.edit.calculatoritem.data', array());
	...
	{
		$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('calculatoritem.id');
		...
		if ($id)
		{
			...
			$authorised = $user->authorise('core.edit', 'com_fisc_calculator') || $authorised = $user->authorise('core.edit.own', 'com_fisc_calculator');
		}
		else
		{
	...
	public function delete($id)
	{
		...
		if (empty($id))
		{
			$id = (int) $this->getState('calculatoritem.id');
		}
		...
		if ($user->authorise('core.delete', 'com_fisc_calculator') !== true)
		{
				throw new \Exception(Text::_('JERROR_ALERTNOAUTHOR'), 403);
		}
